...
function formToYaml(data: OrderFormData): string {
  ...
  if (data.destination.oci) {
    const dstDoc: Record<string, unknown> = { oci: data.destination.oci }
    if (data.destination.pantryRef) {
      const pr = data.destination.pantryRef
      dstDoc.pantryRef = { name: pr.name, ...(pr.namespace ? { namespace: pr.namespace } : {}) }
    }
    doc.destination = dstDoc
  }
  if (data.menuRef) {
    doc.menuRef = { name: data.menuRef.name }
  }
  if (data.source) {
    const srcDoc: Record<string, unknown> = { oci: data.source.oci, version: data.source.version }
    if (data.source.pantryRef) {
      const pr = data.source.pantryRef
      srcDoc.pantryRef = { name: pr.name, ...(pr.namespace ? { namespace: pr.namespace } : {}) }
    }
    doc.source = srcDoc
  }
  if (data.render?.helm) {
...
function yamlToPartialForm(text: string): Omit<OrderFormData, 'name' | 'namespace'> {
  ...
  if (src) {
    ...
    if (srcPantryRef) {
      source.pantryRef = { name: srcPantryRef.name ?? '', namespace: srcPantryRef.namespace ?? '' }
    }
  }
  ...
  if (dst) {
    ...
    if (dstPantryRef) {
      destination.pantryRef = { name: dstPantryRef.name ?? '', namespace: dstPantryRef.namespace ?? '' }
    }
  }
...
function FormView({
  ...
  // Build the OCI ref for the current source, regardless of whether it is a
  // direct OCI URL or a pantry reference.
  function resolveOciRef(): string {
    return formData.source?.oci ?? ''
  }

  function fetchTags(oci: string) {
    ...
    listOCITags(oci, formData.source?.pantryRef)
  ...
  function fetchChartInfo(oci: string, version: string) {
    ...
    getChartInfo(oci, version, formData.source?.pantryRef)
      .then((info) => {
    ...
    <div className={styles.formGrid}>
      ...
      {formData.menuRef ? (
        ...
        <>
          ...
          <div className={styles.fieldGroup}>
            <label className={styles.label}>Pantry <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional — for authentication)</span></label>
            <select
              className={styles.input}
              ...
              onChange={(e) => {
                ...
                if (!name) {
                  onFieldChange('source', { ...(formData.source ?? { oci: '', version: '' }), pantryRef: undefined })
                  return
                }
                const pantry = (pantries ?? []).find((p) => p.name === name)
                const namespace = pantry?.namespace ?? ''
                const oci = formData.source?.oci || pantry?.registry || ''
                onFieldChange('source', { ...(formData.source ?? { oci, version: '' }), oci, pantryRef: { name, namespace } })
              }}
            >
      ...
      <div>
        <button type="button" className={styles.sectionHeader} onClick={() => setIsDestOpen((v) => !v)}>
          ...
          Destination
          {!isDestOpen && formData.destination?.oci && (
            <span className={styles.sectionSummary}>
              {formData.destination.oci}
            </span>
          )}
        </button>
        {isDestOpen && (
          <div className={styles.formGrid} style={{ gap: 10, marginTop: 4 }}>
            <div className={styles.fieldGroup}>
              ...
              <input
                ...
                onChange={(e) => onFieldChange('destination', { ...(formData.destination ?? {}), oci: e.target.value })}
                placeholder={
                  defaultRegistry
            ...
            <div className={styles.fieldGroup}>
              ...
              <select
                ...
                onChange={(e) => {
                  const name = e.target.value
                  if (!name) {
                    onFieldChange('destination', { ...(formData.destination ?? {}), pantryRef: undefined })
                    return
                  }
                  const pantry = (pantries ?? []).find((p) => p.name === name)
                  const namespace = pantry?.namespace ?? ''
                  const oci = formData.destination?.oci || pantry?.registry || ''
                  onFieldChange('destination', { oci, pantryRef: { name, namespace } })
                }}
              >
                <option value="">None</option>
                {(pantries ?? []).map((p) => (

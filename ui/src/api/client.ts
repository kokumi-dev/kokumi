import type { Order, Preparation, OrderFormData, Menu, MenuFormData, Patch, ChartInfo, Pantry, PantryFormData, Repository } from './types'
...
export function listOCITags(ref: string, pantryRef?: { name: string; namespace?: string }): Promise<string[]> {
  let url = `/registry/tags?ref=${encodeURIComponent(ref)}`
  if (pantryRef?.name) {
    url += `&pantryName=${encodeURIComponent(pantryRef.name)}`
    if (pantryRef.namespace) url += `&pantryNamespace=${encodeURIComponent(pantryRef.namespace)}`
  }
...
export function getChartInfo(ref: string, version: string, pantryRef?: { name: string; namespace?: string }): Promise<ChartInfo> {
...
export function listPantryRepositories(
  namespace: string,
  name: string,
): Promise<Repository[]> {
  return request<Repository[]>(`/pantries/${namespace}/${name}/repositories`)
}

export function listPantryTags(
  pantryNamespace: string,
  pantryName: string,
  repoPath: string,
): Promise<string[]> {
  return request<string[]>(`/pantries/${pantryNamespace}/${pantryName}/repositories/${repoPath}/tags`)
}
